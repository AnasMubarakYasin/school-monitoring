...
@else
    @if ($item->answer()->where('status', 'have been checked')->where('student_id', auth()->user()->id)->first())
        <div id="edt_btn"
            class="p-2 text-white bg-teal-400 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm text-center inline-flex items-center dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800 capitalize">
            {{ trans('done') }}
        </div>
    @else
        <a id="edt_btn" href="{{ route('web.student.answer.view', ['materialAndAssignment' => $item]) }}"
            ...
            <span class="sr-only capitalize">Answer Button</span>
        </a>
    @endif
@endif
