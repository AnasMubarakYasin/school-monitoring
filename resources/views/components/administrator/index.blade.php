...
<div class="grid gap-4">
    <div class="flex gap-2 items-center">
        ...
        @if (request()->query('sort') || request()->query('filter') || request()->query('column'))
            ...
            @can('viewAny', App\Models\Administrator::class)
            @endif
        @endcan
        @can('deleteAny', App\Models\Administrator::class)
                            ...
                            @switch($key)
                                ...
                                @default
                                    <div class="flex flex-col gap-1">
                                        ...
                                        <input type="text" id="filter_{{ $key }}" name="filter_{{ $key }}"
                                            value="{{ request()->query("filter_$key") }}"
                        ...
                        @if (in_array($key, $columns))
                            <th scope="col" class="text-base py-3 px-6 capitalize">
                                <div class="flex items-center">
                                    {{ __($value) }}
                                    <a
                                        href="{{ request()->fullUrlWithQuery(['sort' => 'on', "sort_$key" => request()->query("sort_$key", 'desc') == 'desc' ? 'asc' : 'desc']) }}">
