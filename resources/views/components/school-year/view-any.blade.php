...
@section('head')
    @vite('resources/js/components/administrator/index.js')
@endsection
                            ...
                            @switch($key)
                                ...
                                @default
                                    <div class="flex flex-col gap-1">
                                        ...
                                        <input type="text" id="filter_{{ $key }}"
                                            name="filter_{{ $key }}" value="{{ request()->query("filter_$key") }}"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                            placeholder="{{ __("search $value[name]") }}...">
                                    </div>
                            @endswitch
                        </li>
                    @endif
                    ...
                    @if (in_array($key, $columns))
                        <th scope="col" class="p-3 text-base capitalize">
                            <div class="flex items-center w-full h-full">
                                ...
                                <a
                                    href="{{ request()->fullUrlWithQuery(['sort' => 'on', 'sort_name' => $key, 'sort_dir' => request()->query('sort_dir', 'desc') == 'desc' ? 'asc' : 'desc']) }}">
            ...
            @forelse($paginator as $item)
                <tr @class([
                                    ...
                                    <td class="p-3 text-gray-900 dark:text-white whitespace-nowrap">
                                        @if ($value['type'] == 'models')
                                            <div class="flex gap-2">
                                                @foreach ($item->{$key} as $model)
                                                    <button
                                                        class="bg-blue-100 text-blue-800 text-sm font-medium px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300"
                                                        data-modal-target="{{ $key }}-{{ $model->id }}"
                                                        data-modal-toggle="{{ $key }}-{{ $model->id }}">
                                                        {{ $model->type }}
                                                    </button>
                                                    <div id="{{ $key }}-{{ $model->id }}" tabindex="-1"
                                                        aria-hidden="true"
                                                        class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full">
                                                        <x-dynamic-component :component="$model->component" :model="$model"
                                                            class="" />
                                                    </div>
                                                @endforeach
                                            </div>
                    ...
                    <td class="flex justify-center gap-2 p-3 capitalize {{ $loop->last ? 'rounded-br-lg' : '' }}">
                        ...
                        @endcan
                        @can('delete', $item)
                            <form class="contents" action="{{ $delete($item) }}" method="post">
                                @csrf
                                @method('DELETE')
                                <button id="del_btn"
