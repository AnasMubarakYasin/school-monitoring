...
<div class="overflow-x-auto pb-2 rounded-lg">
    <form action="{{ route('web.resource.presence.attendance', ['presence' => $presence]) }}" method="post"
        enctype="multipart/form-data" class="grid gap-4 justify-items-center">
        @csrf
        @method('PATCH')
        <table id="table"
                ...
                @forelse ($presence->students() as $student)
                    <tr @class([
                        ...
                        @foreach ($presence->attendances()->where('student_id', $student->id)->get()->sortBy("number") as $attendance)
                            <td class="p-3  text-gray-900 dark:text-white whitespace-nowrap">
                                <input type="hidden" name="attendances[{{ $attendance->id }}]" value="{{ $attendance->state }}">
                                <input type="checkbox" name="attendances[{{ $attendance->id }}]"
                                    {{ $attendance->state ?? 'indeterminate' }} @checked($attendance->state == 'present')
                                    class="w-4 h-4 cursor-pointer text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 indeterminate:bg-gray-400 dark:bg-gray-700 dark:border-gray-600 dark:hover:ring dark:hover:ring-blue-600 dark:indeterminate:bg-gray-500">
                            </td>
                        @endforeach
                    </tr>
                @empty
